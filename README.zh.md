...
两个命令都会在端口 5433 启动嵌入式 PGlite，执行 schema 同步，然后在 **8637** 端口启动应用。`start:local` 会缓存生产构建，后续启动跳过编译（使用 `pnpm rebuild:local` 强制重建）。Redis 禁用（降级为内存 EventBus）。数据存储在 `.pglite/` 目录，删除即可重置。
